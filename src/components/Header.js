  ...
  return (
    <header className=" bg-white border-b py-4 ">
       ...
       <div className="flex items-center gap-6">
        ...
        {!!session &&(
          <>
          <Link className='flex items-center gap-2' href={'/account'}><img className=' size-7 rounded-full drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]' src={session.user.image} alt='ERROR'/>{session.user.name}</Link>
          <LogOut />
          </>
        )}
        {!session &&(
